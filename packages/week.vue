<template lang="html">
  <div :val="value_">
    <div class="mines">
      <el-radio v-model="type" label="1" size="mini" border style="width:120px;">每周</el-radio>
    </div>
    <div class="mines">
      <el-radio v-model="type" label="5" size="mini" border style="width:120px;">不指定</el-radio>
    </div>
    <div class="mines">
      <el-radio v-model="type" label="2" size="mini" border style="width:120px;">周期</el-radio>
      ...
      <span style="margin-left: 12px; margin-right: 12px;">至星期</span>
      <el-input-number @change="type = '2'" v-model="cycle.end" :min="2" :max="7" size="mini" style="width: 100px;"></el-input-number>
    </div>
    <div class="mines">
      <el-radio v-model="type" label="3" size="mini" border style="width:120px;">循环</el-radio>
      <span style="margin-left: 10px; margin-right: 5px;">从星期</span>
    ...
    <div class="mines">
      <el-radio v-model="type" label="7" size="mini" border style="width:120px;">指定周</el-radio>
    ...
    <div class="mines">
      <el-radio v-model="type" label="6" size="mini" border style="width:120px;">本月最后一个</el-radio>
      <span style="margin-left: 24px; margin-right: 5px;">星期</span>
      <el-input-number @change="type = '6'" v-model="last" :min="1" :max="7" size="mini" style="width: 100px;"></el-input-number>
    </div>
    <div class="mines">
      <el-radio v-model="type" label="4" size="mini" border style="width:120px;">指定</el-radio>
      <el-checkbox-group v-model="appoint" style="margin-left: 10px; margin-top:10px; line-height: 25px;">
        <el-checkbox @change="type = '4'" v-for="i in 7" :key="String(i)" :label="String(i)"></el-checkbox>
      </el-checkbox-group>
    </div>
  </div>
</template>
...
<script>
import { handleTimeType } from './method.js'
export default {
  props: {
    value: {
      type: String,
  ...
  computed: {
    value_() {
      let result = handleTimeType(this, this.type, 'week')
      return result
    }
  },
  watch: {
...
.mines ::v-deep .el-input-number__increase{
  height: 26px;
  line-height: 26px;
}
.mines ::v-deep .el-input-number__decrease{
  height: 26px;
  line-height: 26px;
}
</style>
